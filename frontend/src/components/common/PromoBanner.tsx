...
function discountLabel(c: Coupon) {
  return c.type === 'PERCENTAGE'
    ? `${c.value}% OFF`
    : `R$ ${Number(c.value).toFixed(2)} OFF`;
}
  ...
  return (
    <div className="relative bg-gradient-to-r from-accent via-rose-500 to-pink-500 text-white overflow-hidden">
      ...
      <div className="absolute inset-0 opacity-10">
        <div
          className="absolute inset-0 bg-[linear-gradient(110deg,transparent_25%,rgba(255,255,255,.3)_50%,transparent_75%)] animate-[shimmer_3s_infinite]"
      ...
      <div className="relative max-w-7xl mx-auto px-4 py-2.5 flex items-center justify-center gap-3 text-sm">
        ...
        <div className="flex items-center gap-2 min-w-0">
          ...
          <span className="hidden sm:inline truncate">
            {current.description || `Use code ${current.code}`}
          </span>
          <span className="font-mono bg-white/20 px-2 py-0.5 rounded text-xs font-bold tracking-widest flex-shrink-0">
            {current.code}
          </span>
          {current.expires_at && (
