...
export function AdminCoupons() {
  ...
  const buildPayload = () => {
    ...
    if (form.min_order_amount)
      p.min_order_amount = Number(form.min_order_amount);
    if (form.max_discount_amount)
      p.max_discount_amount = Number(form.max_discount_amount);
                ...
                coupons.map((c) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-5 py-3.5 hidden md:table-cell">
                        ...
                        {c.min_order_amount && (
                          <p className="text-xs text-gray-400">
                            min R$ {Number(c.min_order_amount).toFixed(2)}
                          </p>
                        )}
                      </td>
                      ...
                      <td className="px-5 py-3.5 text-center">
                        <span
                          className={`inline-block text-xs font-medium px-2.5 py-0.5 rounded-full border ${st.cls}`}
                        >
                          {st.label}
                        </span>
                      </td>
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg mx-4 p-6">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div>
                ...
                <p className="font-medium">{valLabel(viewTarget)}</p>
              </div>
              <div>
                <p className="text-gray-500">Min Order</p>
                <p className="font-medium">
                  {viewTarget.min_order_amount
                    ? `R$ ${Number(viewTarget.min_order_amount).toFixed(2)}`
                    : '—'}
                </p>
              </div>
              <div>
                ...
                <p className="font-medium">
                  {viewTarget.max_discount_amount
                    ? `R$ ${Number(viewTarget.max_discount_amount).toFixed(2)}`
                    : '—'}
                </p>
              </div>
              <div>
                ...
                <p className="font-medium">
                  {viewTarget.usage_count}
                  {viewTarget.usage_limit ? ` / ${viewTarget.usage_limit}` : ''}
                </p>
              </div>
              <div>
                <p className="text-gray-500">Per User Limit</p>
                <p className="font-medium">
                  {viewTarget.per_user_limit ?? '—'}
                </p>
              </div>
              <div>
                <p className="text-gray-500">Starts</p>
                <p className="font-medium">
                  {formatDate(viewTarget.starts_at)}
                </p>
              </div>
              <div>
                <p className="text-gray-500">Expires</p>
                <p className="font-medium">
                  {formatDate(viewTarget.expires_at)}
                </p>
              </div>
              <div>
                <p className="text-gray-500">Visibility</p>
                <p className="font-medium">
                  {viewTarget.is_public ? 'Public' : 'Private'}
                </p>
              </div>
              <div>
              ...
              <div className="flex flex-wrap gap-6 pt-2">
                <label className="flex items-center gap-3 cursor-pointer">
                  <button
                    ...
                    className={`w-10 h-6 rounded-full transition-colors relative ${form.is_active ? 'bg-accent' : 'bg-gray-300'}`}
                  >
                    <span
                      className={`absolute top-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform ${form.is_active ? 'left-[18px]' : 'left-0.5'}`}
                    />
                  </button>
                ...
                <label className="flex items-center gap-3 cursor-pointer">
                  <button
                    ...
                    className={`w-10 h-6 rounded-full transition-colors relative ${form.is_public ? 'bg-blue-500' : 'bg-gray-300'}`}
                  >
                    <span
                      className={`absolute top-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform ${form.is_public ? 'left-[18px]' : 'left-0.5'}`}
                    />
                  </button>
                  <span className="text-sm font-medium text-gray-700">
